...
while (1==1)
    ...
    if (choice==1)
        ...
        for k=1:20
           ...
           % I = imread('C:\Users\user\Desktop\disease (%d).jpg', k);
    ...
    if (choice==2)
        ...
        if result == 1
            ...
            disp(' Disease Detected');
             msg = ('Bacterial Blight disease detected');
            msgbox(msg,'the result is..');
   msg ='(i)Use balanced amounts of plant nutrients, especially nitrogen. ';
   msg=[msg newline '(ii) Apply N in three split doses, 50% basal, 25% in tillering phase and 25%N in panicle initiation stage']
   msg=[msg newline '(iii)Foliar spray of 0.05 g Streptocycline and 0.05 g Copper Sulfate']
   msgbox(msg,'the suggestion is..');
            
        else
            helpdlg(' Disease not Detected ');
            disp('Disease not Detected');
        end
    end
    if (choice==3)
        close all;
